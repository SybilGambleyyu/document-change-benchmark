...
DCAB does not claim client rendering/runtime compatibility or universal security policy. It is a narrow, tool-neutral static-review benchmark. In particular, the VML pair does not resolve or follow its direct link; the fragmented complex-field pair does not resolve/import a source, update/evaluate a field, open Word, or assert client processing; the task-pane pair does not install, retrieve, authenticate, or execute an add-in or assert that a pane opens; and the `commentsExtended` pair does not open Word, resolve a thread, infer an identity, synchronize with a service, or assert a client comment UI behavior. See the [repository README](https://github.com/SybilGambleyyu/document-change-benchmark) and [research notes](https://github.com/SybilGambleyyu/document-change-benchmark/blob/v0.13.0/RESEARCH.md) for contract details and limits.
