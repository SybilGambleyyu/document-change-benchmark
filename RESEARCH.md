...
The initial releases intentionally do not claim coverage for every WordprocessingML surface. Candidate future work includes linked pictures, legacy VML hyperlinks, mail merge, other external document dependencies, ActiveX controls, document variables, permission ranges, modern comments, task-pane web extensions, and style-resolution semantics. Each requires the same standard: a narrowly defined static fact, safe synthetic pair generation, a target-free oracle, and a credible independent reader/consumer test.
